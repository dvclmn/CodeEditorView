...
class CodeStorage: NSTextStorage {
    ...
    private func getLanguage(from location: Int) -> LanguageConfiguration {
        
        if let codeBlockManager = (delegate as? CodeStorageDelegate)?.codeBlockManager {
            return codeBlockManager.languageAndRangeContaining(location: location).language
        } else {
            return .none // Make sure you have a default language defined
        }
    }
    ...
    override func replaceCharacters(in range: NSRange, with str: String) {
        ...
        let isOpenBracket = deletedToken.token.isOpenBracket
        let isWithinTextBounds = range.location + 1 < length
        let language = getLanguage(from: range.location)
        let isSingleCharBracket = language.lexeme(of: deletedToken.token)?.count == 1
        let nextTokenIsMatchingBracket = token(at: range.location + 1).token?.token == deletedToken.token.matchingBracket
